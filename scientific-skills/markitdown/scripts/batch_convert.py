...
def convert_file(
    md: MarkItDown, file_path: Path, output_dir: Path, verbose: bool = False
) -> tuple[bool, str, str]:
    """
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            results["details"].append(
                {"file": path, "success": success, "message": message}
            )
...
Examples:
  ...
  python batch_convert.py input/ output/ --plugins
    ...
    parser.add_argument(
        "--plugins", "-p", action="store_true", help="Enable MarkItDown plugins"
    )
