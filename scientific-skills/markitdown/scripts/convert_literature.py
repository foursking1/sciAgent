...
def create_index(papers: List[Dict], output_dir: Path):
    ...
    papers_sorted = sorted(
        papers, key=lambda x: (x.get("year", "9999"), x.get("title", ""))
    )
...
Filename Conventions:
  ...
  Examples:
    ...
    parser.add_argument(
        "output_dir", type=Path, help="Output directory for Markdown files"
    )
    parser.add_argument(
        "--organize-by-year",
        "-y",
    ...
    for pdf_file in pdf_files:
        success, metadata = convert_paper(
            md, pdf_file, args.output_dir, args.organize_by_year
        )
