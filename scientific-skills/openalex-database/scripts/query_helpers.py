...
def find_author_works(
    author_name: str, client: OpenAlexClient, limit: Optional[int] = None
) -> List[Dict[str, Any]]:
    """
    ...
    author_response = client._make_request(
        "/authors", params={"search": author_name, "per-page": 1}
    )
    ...
    works_params = {"filter": f"authorships.author.id:{author_id}", "per-page": 200}
...
def find_institution_works(
    institution_name: str, client: OpenAlexClient, limit: Optional[int] = None
) -> List[Dict[str, Any]]:
    """
    ...
    inst_response = client._make_request(
        "/institutions", params={"search": institution_name, "per-page": 1}
    )
    ...
    works_params = {"filter": f"authorships.institutions.id:{inst_id}", "per-page": 200}
...
def get_open_access_papers(
    ...
    params = {"search": search_term, "filter": filter_str, "per-page": min(limit, 200)}
...
def analyze_research_output(
    ...
    entity_response = client._make_request(
        endpoint, params={"search": entity_name, "per-page": 1}
    )
    ...
    # Total works
    works_response = client.search_works(filter_params=filter_params, per_page=1)
    ...
    # Top topics
    topics = client.group_by("works", "topics.id", filter_params=filter_params)

    # OA percentage
    oa_works = client.search_works(
        filter_params={**filter_params, "is_oa": "true"}, per_page=1
    )
