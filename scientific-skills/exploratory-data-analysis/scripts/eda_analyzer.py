...
def analyze_bioinformatics(filepath, extension):
    ...
    try:
        if extension in ["fasta", "fa", "fna"]:
            ...
            lengths = [len(seq) for seq in sequences]
            qualities = [
                sum(seq.letter_annotations["phred_quality"]) / len(seq)
        ...
        results["error"] = (
            f"Required library not installed (try: pip install biopython): {e}"
        )
    except Exception as e:
        results["error"] = f"Analysis error: {e}"
...
def analyze_imaging(filepath, extension):
    ...
    try:
        ...
        results["error"] = (
            f"Required library not installed (try: pip install pillow): {e}"
        )
    except Exception as e:
        results["error"] = f"Analysis error: {e}"
...
def generate_markdown_report(analysis, output_path=None):
    ...
    lines.append(
        f"- **File Size:** {basic['size_human']} ({basic['size_bytes']:,} bytes)"
    )
