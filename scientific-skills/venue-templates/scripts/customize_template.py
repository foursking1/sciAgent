...
def customize_template(template_path, output_path, **kwargs):
    ...
    replacements = {
        ...
        "email": (
            [r"first\.author@university\.edu", r"\[email protected\]"],
            kwargs.get("email", ""),
        ),
    }
    ...
    if modified:
        ...
        print(
            "   No customizations applied (no matching placeholders found or no values provided)"
        )
...
def interactive_mode():
    ...
    authors = input(
        "Authors (e.g., 'John Doe, Jane Smith') (press Enter to skip): "
    ).strip()
