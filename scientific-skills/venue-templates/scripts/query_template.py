    ...
    for cat_name, category in TEMPLATES.items():
        ...
        for temp_id, template in category.items():
            ...
            if venue:
                ...
                if (
                    venue_lower not in temp_id
                    and venue_lower not in template.get("full_name", "").lower()
                ):
                    continue
...
Examples:
  ...
  %(prog)s --venue "NeurIPS" --requirements
    ...
    parser.add_argument(
        "--venue", type=str, help='Venue name (e.g., "Nature", "NeurIPS")'
    )
    parser.add_argument(
        "--type",
        type=str,
    ...
    parser.add_argument(
        "--list-all", action="store_true", help="List all available templates"
    )
    parser.add_argument(
        "--requirements",
        action="store_true",
    ...
    results = search_templates(
        venue=args.venue, template_type=args.type, keyword=args.keyword
    )
