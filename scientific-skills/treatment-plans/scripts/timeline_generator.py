...
def extract_timeline_info(content: str) -> Dict[str, List[Tuple[str, str]]]:
    ...
    for pattern in apt_patterns:
        ...
        for timeframe, visit_type, details in matches:
            timeline_data["appointments"].append(
                (timeframe.strip(), f"{visit_type}{details}".strip())
            )
    ...
    for pattern in milestone_patterns:
        ...
        for timeframe, event_type, details in matches:
            timeline_data["milestones"].append(
                (timeframe.strip(), f"{event_type}{details}".strip())
            )
...
def create_visual_timeline(
    timeline_data: Dict, output_file: Path, start_date: str = None
):
    ...
    for i, phase in enumerate(phases):
        ...
        ax.barh(
            y_position - i,
            duration,
            left=mdates.date2num(phase["start"]),
            height=0.6,
            color="steelblue",
            alpha=0.7,
            edgecolor="black",
...
Note: Visual timeline generation requires matplotlib.
  Install with: pip install matplotlib
    ...
    parser.add_argument(
        "--start", help="Start date for timeline (YYYY-MM-DD format, default: today)"
    )
    ...
    if total_items == 0:
        print(
            "\nWarning: No timeline information detected in treatment plan.",
            file=sys.stderr,
        )
        print(
            "The plan may not contain structured timeline/schedule sections.",
            file=sys.stderr,
        )
