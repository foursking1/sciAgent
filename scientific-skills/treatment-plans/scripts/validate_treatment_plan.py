...
VALIDATION_CHECKS = {
    ...
    "evidence_based": {
        ...
        "patterns": [
            (
                r"guideline|evidence|study|trial|research",
    ...
    "patient_centered": {
        ...
        "patterns": [
            (
                r"patient.*preference|shared decision|patient.*value|patient.*priority",
                "Patient preferences",
            ),
    ...
    "safety": {
        ...
        "patterns": [
            (
                r"adverse.*effect|side effect|risk|complication",
                "Adverse effects mentioned",
            ),
            (
                r"monitoring|warning sign|emergency|when to call",
...
def display_validation_results(
    ...
    print("-" * 70)
    print(
        f"Validation checks passed: {total_passed}/{total_checks} ({quality_pct:.0f}%)"
    )
    ...
    if has_timeframes:
        ...
        print(
            "   Recommendation: Add specific timeframes (e.g., 'within 3 months', '8 weeks')"
        )
    ...
    if has_metrics:
        ...
        print(
            "   Recommendation: Include specific measurable targets (HbA1c <7%, BP <130/80)"
        )
    ...
    if results["smart_goals"][0] < results["smart_goals"][1]:
        recommendations.append(
            "Ensure all goals meet SMART criteria (Specific, Measurable, Achievable, Relevant, Time-bound)"
        )

    # Evidence-based
    if results["evidence_based"][0] == 0:
        recommendations.append(
            "Add evidence-based rationale and cite clinical practice guidelines"
        )

    # Patient-centered
    if results["patient_centered"][0] < results["patient_centered"][1]:
        recommendations.append(
            "Incorporate patient preferences and functional quality-of-life goals"
        )

    # Safety
    if results["safety"][0] < results["safety"][1]:
        recommendations.append(
            "Include comprehensive safety monitoring and risk mitigation strategies"
        )

    # Medication documentation
    if results["medication"][0] < results["medication"][1]:
        recommendations.append(
            "Document medications with specific doses, frequencies, and rationales"
        )

    if not has_icd10:
        recommendations.append(
            "Add ICD-10 diagnosis codes for billing and documentation support"
        )
    ...
    if recommendations:
        ...
        print(
            "None - Treatment plan demonstrates excellent quality across all criteria!"
        )
...
Exit Codes:
  ...
  2 - File error or invalid arguments
    ...
    parser.add_argument(
        "file", type=Path, help="Treatment plan file to validate (.tex format)"
    )
