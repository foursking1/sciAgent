...
class GapAnalyzer:
    ...
    def _search_for_procedure(
        self, documents: List[Tuple[Path, str]], clause_id: str, proc_info: Dict
    ):
    ...
    def _search_for_document(
        self, documents: List[Tuple[Path, str]], doc_name: str, keywords: List[str]
    ):
        """Search for key documents."""
        matches = []
        for file_path, content in documents:
    ...
    def _generate_report(self) -> Dict:
        ...
        for doc_name, keywords in KEY_DOCUMENTS.items():
            if doc_name not in self.found_documents:
                missing_documents.append({"document": doc_name, "keywords": keywords})
        ...
        report = {
            ...
            "recommendations": self._generate_recommendations(
                missing_procedures, missing_documents
            ),
        }
    ...
    def _generate_recommendations(
        self, missing_procedures: List[Dict], missing_documents: List[Dict]
    ) -> List[str]:
        ...
        if missing_procedures:
            ...
            high_priority_missing = [
                p for p in missing_procedures if p["clause"] in high_priority_clauses
            ]
...
def main():
    ...
    parser.add_argument("--output", help="Output file path for JSON report (optional)")
