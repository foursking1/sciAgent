...
def simple_algorithm_to_tikz(algorithm_text, output_file="algorithm.tex"):
    ...
    lines = [
        line.strip() for line in algorithm_text.strip().split("\n") if line.strip()
    ]
            ...
            if "->" in line:
                ...
                for branch in branches:
                    ...
                    if branch.startswith("YES:"):
                        ...
                        arrow_defs.append(
                            f"\\draw [{arrow_style}] ({node_id}) -- node[above] {{Yes}} ({yes_id});"
                        )
                        ...
                        arrow_defs.append(
                            f"\\draw [{arrow_style}] ({node_id}) -- node[above] {{No}} ({no_id});"
                        )
    ...
    def add_node(node_id, position_rel=None):
        ...
        if node_type == "decision":
            ...
            if yes_target:
                ...
                arrow_style = (
                    "urgentarrow" if target_node.get("urgency") == "urgent" else "arrow"
                )
                arrow_defs.append(
...
def main():
    parser = argparse.ArgumentParser(
        description="Build clinical decision tree flowcharts"
    )
    parser.add_argument(
        "-i", "--input", type=str, default=None, help="Input file (JSON format)"
    )
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        default="clinical_algorithm.tex",
        help="Output .tex file",
    )
    parser.add_argument(
        "--example", action="store_true", help="Generate example algorithm"
    )
    parser.add_argument(
        "--text",
        type=str,
    ...
    if args.example:
        ...
        print(
            "No input provided. Use --example to generate example, --text for simple text, or -i for JSON input."
        )
        print("\nSimple text format:")
