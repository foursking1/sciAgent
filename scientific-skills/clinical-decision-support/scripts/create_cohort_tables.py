...
def calculate_p_value(data, variable, group_col="group", var_type="categorical"):
    ...
    if var_type == "categorical":
        ...
        raise ValueError(
            "var_type must be 'categorical', 'continuous_normal', or 'continuous_nonnormal'"
        )
...
def format_continuous_variable(data, variable, group_col, distribution="normal"):
    ...
    # Calculate p-value
    var_type = (
        "continuous_normal" if distribution == "normal" else "continuous_nonnormal"
    )
...
def generate_baseline_table(data, group_col="group", output_file="table1_baseline.csv"):
    ...
    header = {
        ...
        **{
            group: f"{group} (n={len(data[data[group_col]==group])})"
            for group in groups
        },
        "p_value": "p-value",
    }
    ...
    if "age" in data.columns:
        age_results = format_continuous_variable(
            data, "age", group_col, distribution="nonnormal"
        )
    ...
    if "sex" in data.columns:
        table_rows.append(
            {"Characteristic": "Sex, n (%)", **{g: "" for g in groups}, "p_value": ""}
        )
...
def generate_efficacy_table(data, group_col="group", output_file="table2_efficacy.csv"):
    ...
    if "best_response" in data.columns:
        for group in groups:
            group_data = data[data[group_col] == group]
            cr_pr = (
                (group_data["best_response"] == "CR")
                | (group_data["best_response"] == "PR")
            ).sum()
        ...
        contingency = pd.crosstab(
            data["best_response"].isin(["CR", "PR"]), data[group_col]
        )
        _, p_value, _, _ = stats.chi2_contingency(contingency)
    ...
    if "best_response" in data.columns:
        ...
        # P-value
        contingency = pd.crosstab(
            data["best_response"].isin(["CR", "PR", "SD"]), data[group_col]
        )
        _, p_value, _, _ = stats.chi2_contingency(contingency)
...
def generate_safety_table(
    data, ae_columns, group_col="group", output_file="table3_safety.csv"
):
    """
...
def generate_latex_table(df, caption, label="table"):
    ...
    for _, row in df.iterrows():
        ...
        data_row = [first_col] + [
            str(val) if pd.notna(val) else "—" for val in row.iloc[1:]
        ]
    ...
    data = pd.DataFrame(
        {
            ...
            "best_response": np.random.choice(
                ["CR", "PR", "SD", "PD"], n, p=[0.05, 0.35, 0.40, 0.20]
            ),
            "fatigue_grade": np.random.choice([0, 1, 2, 3], n, p=[0.3, 0.4, 0.2, 0.1]),
            "nausea_grade": np.random.choice(
                [0, 1, 2, 3], n, p=[0.4, 0.35, 0.20, 0.05]
            ),
            "neutropenia_grade": np.random.choice(
                [0, 1, 2, 3, 4], n, p=[0.5, 0.2, 0.15, 0.10, 0.05]
            ),
        }
    )
...
def main():
    ...
    parser.add_argument(
        "--example", action="store_true", help="Generate tables using example data"
    )
