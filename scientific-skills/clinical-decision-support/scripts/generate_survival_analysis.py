...
def generate_kaplan_meier_plot(
    ...
    for i, group in enumerate(groups):
        ...
        kmf.plot_survival_function(
            ax=ax, ci_show=True, color=colors[i % len(colors)], linewidth=2, alpha=0.8
        )
...
def generate_number_at_risk_table(
    data, time_col="time", event_col="event", group_col="group", time_points=None
):
    """
...
def calculate_hazard_ratio(
    data, time_col="time", event_col="event", group_col="group", reference_group=None
):
    """
    ...
    cph = CoxPHFitter()
    cph.fit(
        data_cox[[time_col, event_col, "group_binary"]],
...
def generate_report(data, output_dir, prefix="survival"):
    ...
    with open(output_dir / f"{prefix}_statistics.txt", "w") as f:
        ...
        for group in groups:
            ...
            try:
                ...
                surv_24m = (
                    kmf.survival_function_at_times(24).values[0]
                    if data["time"].max() >= 24
                    else None
                )
            except:
    ...
    with open(output_dir / f"{prefix}_latex_table.tex", "w") as f:
        ...
        f.write("\\end{tabular}\n")
        f.write(
            f"\\caption{{Survival outcomes by group (log-rank p={logrank_p:.3f})}}\n"
        )
        f.write("\\end{table}\n")
...
def main():
    ...
    parser.add_argument(
        ...
        "--title",
        type=str,
        default="Kaplan-Meier Survival Curve",
        help="Plot title",
    )
    parser.add_argument(
        "-x", "--xlabel", type=str, default="Time (months)", help="X-axis label"
    )
    parser.add_argument(
        "-y", "--ylabel", type=str, default="Survival Probability", help="Y-axis label"
    )
    parser.add_argument(
        "--time-col", type=str, default="time", help="Column name for time variable"
    )
    parser.add_argument(
        "--event-col", type=str, default="event", help="Column name for event indicator"
    )
    parser.add_argument(
        "--group-col",
        type=str,
