...
class CDSValidator:
    ...
    def check_required_sections(self):
        ...
        if is_cohort:
            ...
            if missing:
                self.warnings.append(
                    f"Cohort analysis may be missing sections: {', '.join(missing)}"
                )
            else:
        ...
        if is_recommendation:
            ...
            if missing:
                self.errors.append(
                    f"Recommendation document missing required sections: {', '.join(missing)}"
                )
            else:
    ...
    def check_evidence_citations(self):
        ...
        recommendations = re.findall(
            rec_pattern, self.content, re.IGNORECASE | re.DOTALL
        )
        ...
        for i, (_, rec_text) in enumerate(recommendations):
            has_citation = any(
                re.search(pattern, rec_text) for pattern in citation_patterns
            )
        ...
        if uncited_recommendations:
            self.warnings.append(
                f"Found {len(uncited_recommendations)} recommendations without citations"
            )
            for rec in uncited_recommendations[:3]:  # Show first 3
                self.warnings.append(f"  - {rec}")
        else:
            self.info.append(
                f"All {len(recommendations)} recommendations have citations"
            )
    ...
    def check_recommendation_grading(self):
        ...
        conditional_pattern = (
            r"(conditional|weak|we suggest|may consider|could consider)"
        )

        strong_count = len(re.findall(strong_pattern, self.content, re.IGNORECASE))
        conditional_count = len(
            re.findall(conditional_pattern, self.content, re.IGNORECASE)
        )
        ...
        if grades:
            ...
            self.warnings.append(
                "No GRADE-style recommendation grading found (1A, 1B, 2A, etc.)"
            )

        if strong_count > 0 or conditional_count > 0:
            self.info.append(
                f"Recommendation language: {strong_count} strong, {conditional_count} conditional"
            )
        else:
            self.warnings.append(
                "No clear recommendation strength language (strong/conditional) found"
            )
    ...
    def check_statistical_reporting(self):
        ...
        if not p_values:
            self.warnings.append(
                "No p-values found - statistical significance not reported"
            )
        else:
    ...
    def check_hipaa_identifiers(self):
        ...
        for identifier_type, pattern in identifiers.items():
            ...
            if matches:
                found_identifiers.append(
                    f"{identifier_type}: {len(matches)} instance(s)"
                )
        ...
        if found_identifiers:
            ...
            self.errors.append(
                "  ** Ensure proper de-identification before distribution **"
            )
        else:
    ...
    def check_biomarker_nomenclature(self):
        ...
        for gene in gene_names:
            ...
            if gene in self.content:
                if (
                    f"\\textit{{{gene}}}" not in self.content
                    and f"\\emph{{{gene}}}" not in self.content
                ):
                    if ".tex" in self.filepath.suffix:
                        issues.append(
                            f"{gene} should be italicized in LaTeX (\\textit{{{gene}}})"
                        )
        ...
        if hgvs_mutations:
            self.info.append(
                f"Found {len(hgvs_mutations)} HGVS protein nomenclature (e.g., p.L858R)"
            )

        # Warn about non-standard mutation format
        if "EGFR mutation" in self.content and "exon" not in self.content.lower():
            self.warnings.append(
                "EGFR mutation mentioned - specify exon/variant (e.g., exon 19 deletion)"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate clinical decision support documents"
    )
    parser.add_argument(
        "input_file", type=str, help="Document to validate (.tex, .md, .txt)"
    )
    parser.add_argument(
        "-o", "--output", type=str, default=None, help="Save validation report to file"
    )
    parser.add_argument(
