...
class TestDataSourceModel:
    ...
    async def test_create_vector_store_data_source(self, async_session: AsyncSession, test_user: User):
        """测试创建向量库类型数据源"""
        ds = DataSource(
            user_id=test_user.id,
            name="Test Vector Store",
            type="vector_store",
            config={"collection": "documents", "embedding_model": "text-embedding-3-small"}
        )
        async_session.add(ds)
        await async_session.commit()
        await async_session.refresh(ds)
    ...
    async def test_data_source_user_relationship(self, async_session: AsyncSession, test_user: User):
        """测试数据源与用户的关系"""
        ds = DataSource(
            user_id=test_user.id,
    ...
    async def test_data_source_config_json_field(self, async_session: AsyncSession, test_user: User):
        """测试config字段存储复杂JSON"""
        complex_config = {
            "host": "localhost",
            "port": 5432,
    ...
    async def test_data_source_default_is_active(self, async_session: AsyncSession, test_user: User):
        """测试数据源默认是激活状态"""
        ds = DataSource(
            user_id=test_user.id,
            name="Active Test",
    ...
    async def test_data_source_can_be_deactivated(self, async_session: AsyncSession, test_user: User):
        """测试数据源可以被停用"""
        ds = DataSource(
            user_id=test_user.id,
    ...
    async def test_data_source_created_at_timestamp(self, async_session: AsyncSession, test_user: User):
        """测试数据源创建时间戳"""
        ds = DataSource(
            user_id=test_user.id,
            name="Timestamp Test",
    ...
    async def test_multiple_data_sources_for_user(self, async_session: AsyncSession, test_user: User):
        """测试一个用户可以有多个数据源"""
        for i in range(3):
            ds = DataSource(
                user_id=test_user.id,
