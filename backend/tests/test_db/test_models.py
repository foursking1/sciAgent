...
class TestUserModel:
    ...
    async def test_user_model_create(self, async_session: AsyncSession):
        """测试用户模型创建"""
        user = User(email="test@example.com", password_hash="hashed_value")
        async_session.add(user)
        await async_session.commit()
...
class TestMessageModel:
    ...
    async def test_message_session_relationship(
...
class TestFileModel:
    ...
    async def test_file_session_files_relationship(
        self, async_session: AsyncSession, test_session: Session
    ):
        """测试会话的文件列表关联"""
        # 创建多个文件
...
class TestIntegration:
    ...
    async def test_full_session_workflow(self, async_session: AsyncSession):
        ...
        # 2. 创建会话
        session = Session(
            id="session_workflow", user_id=user.id, working_dir="/tmp/workflow_session"
        )
        async_session.add(session)
        await async_session.commit()
        await async_session.refresh(session)
